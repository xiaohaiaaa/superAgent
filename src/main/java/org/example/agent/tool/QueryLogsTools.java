...
import org.example.dto.LogTopicInfo;
import org.example.dto.LogTopicsOutput;
import org.example.dto.QueryLogsOutput;
...
public class QueryLogsTools {
    ...
    public String getAvailableLogTopics() {
        ...
        try {
            ...
            dbSlowQuery.setDescription("数据库慢查询日志");
    ...
    private List<LogEntry> buildApplicationLogs(Instant now, String query, int limit) {
        ...
        if (query.contains("error") || query.contains("fatal") || query.contains("500")) {
            ...
            dbError.setMessage("数据库连接池耗尽: Cannot acquire connection from pool, active: 50/50, waiting: 23, timeout: 30000ms");
            ...
            oomError.setMessage("java.lang.OutOfMemoryError: Java heap space at com.example.order.service.OrderService.processLargeOrder(OrderService.java:156)");
            ...
            for (int i = 0; i < 3; i++) {
                ...
                httpError.setMessage(String.format("HTTP 500 Internal Server Error: /api/v1/users/profile, 耗时: %dms, 错误: Database query timeout", 5200 + i * 300));
    ...
    private List<LogEntry> buildDatabaseSlowQueryLogs(Instant now, String query, int limit) {
        ...
        slowQuery1.setMessage("慢查询: SELECT * FROM orders WHERE user_id = ? AND status IN (?, ?, ?) ORDER BY created_at DESC LIMIT 100, 执行时间: 3.2s, 扫描行数: 1,245,678");
        ...
        slowQuery2.setMessage("慢查询: SELECT u.*, p.* FROM users u LEFT JOIN user_profiles p ON u.id = p.user_id WHERE u.last_login > ?, 执行时间: 2.8s, 全表扫描");
    ...
    private List<LogEntry> buildSystemEventsLogs(Instant now, String query, int limit) {
        ...
        if (query.contains("restart") || query.contains("crash") || query.contains("oom_kill")) {
            ...
            restartEvent.setMessage("Pod 重启事件: pod-order-service-5c7d8e9f1-m3n2p, 原因: OOMKilled, 容器退出码: 137, 重启次数: 3");
            ...
            oomKillEvent.setMessage("OOM Killer 触发: 进程 java (PID: 12345) 被杀死, 内存使用: 3.9GB, 内存限制: 4GB");
