...
public class VectorIndexServiceImpl implements VectorIndexService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(VectorIndexServiceImpl.class);
    ...
    public void indexSingleFile(String filePath) throws Exception {
        ...
        logger.info("开始读取文件: {}, 文件大小: {} MB", path, fileSize / 1024 / 1024);

        // 优化内存使用：对于大文件使用更高效的方式
        try {
            // 使用 UTF-8 编码读取，确保正确处理中文字符
            content = Files.readString(path, java.nio.charset.StandardCharsets.UTF_8);
            logger.info("文件读取完成: {}, 内容长度: {} 字符", path, content.length());
        } catch (OutOfMemoryError e) {
            logger.error("内存不足，无法读取文件: {}", path);
            throw new RuntimeException("文件太大，内存不足。请考虑将文件拆分成较小的部分。", e);
        } catch (Exception e) {
        ...
        // 3. 文档分片
        List<DocumentChunk> chunks = chunkService.chunkDocumentV2(content, path.toString());
